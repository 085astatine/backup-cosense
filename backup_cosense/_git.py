...
class Commit:
    ...
    def backup_info(self) -> Optional[BackupInfoJSON]:
        ...
        try:
            info = parse_json(body, schema=jsonschema_backup_info())
...
class Git:
    ...
    def switch(
        ...
        *,
        ...
        if allow_orphan and self._branch not in self.branches():
            ...
            self._logger.info(f'create orphan branch "{self._branch}" before commit')
            ...
            self._logger.info(f'switch git branch from "{branch}" to "{self._branch}"')
            _execute_git_command(
    ...
    def init(self) -> None:
        ...
        if self.exists():
            self._logger.error(f'git repository "{self.path}" already exists')
            return
        # mkdir
    ...
    def ls_files(self) -> list[pathlib.Path]:
        ...
        return [self.path.joinpath(path) for path in process.stdout.split("\0") if path]
    ...
    def latest_commit_timestamp(self) -> Optional[int]:
        # check if the repository exists
        if not self.exists():
            self._logger.warning(f'git repository "{self.path}" does not exist')
            return None
